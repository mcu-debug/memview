    ...
    public static debuggerStatusChanged(
        ...
        for (const [_id, doc] of Object.entries(DualViewDoc.allDocuments)) {
            ...
            if (doc.sessionId !== sessionId) {
                if (
                    (status === DebugSessionStatus.Started ||
                    ...
                    if (status === DebugSessionStatus.Stopped) {
                        doc.markAsStale();
                        doc.sessionStatus = DocDebuggerStatus.Stopped;
                    }
                }
            } else if (status !== DebugSessionStatus.Initializing) {
                doc.isReady = status === DebugSessionStatus.Stopped;
                if (status === DebugSessionStatus.Stopped) {
                    doc.markAsStale();
                    doc.sessionStatus = DocDebuggerStatus.Stopped;
                } else if (status === DebugSessionStatus.Terminated) {
