...
export interface IGetMemoryCommand {
...
export class WebviewDoc {
    ...
    private static memoryFetcher: IGetMemoryCommand;
    public static init(arg: IGetMemoryCommand) {
        WebviewDoc.memoryFetcher = arg;
    }
    ...
    getMoreMemory(addr: bigint, nBytes: number): Promise<Uint8Array> {
        ...
        const promise = WebviewDoc.memoryFetcher.getMoreMemory(msg);
    ...
    static setCurrentDoc(docOrId: WebviewDoc | string) {
        const id: string = (typeof docOrId === 'string') ? (docOrId as string) : (docOrId as WebviewDoc).sessionId;
        const doc = WebviewDoc.allDocuments[id];
        if (doc) {
    ...
    public getValue(addr: bigint, forceOld: boolean): number | Promise<number> {
        ...
        if (!page || page.stale || !page.buffer.length) {
            ...
            return new Promise((resolve) => {
                this.parentDoc.getMoreMemory(blockAddr, MemPages.BucketSize).then((buf) => {
